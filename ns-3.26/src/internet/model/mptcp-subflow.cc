...
// The following function is uncommented by kashif to fork mptcp subflow
//Ptr<MpTcpSubflow>
...
MpTcpSubflow::MpTcpSubflow(const MpTcpSubflow& sock)
  ...
  addrAdvertised(sock.addrAdvertised)
{
  NS_LOG_FUNCTION (this << &sock);
...
MpTcpSubflow::SendPacket(TcpHeader header, Ptr<Packet> p)
{
  ...
  //! if we send data...
   ...
   //This condition is added by kashif in following if() -> !GetTcpOption(header, join))
...
{
  NS_LOG_FUNCTION(this << header <<"Sending packet starting at SSN [" << ssnHead.GetValue() << "] with len=" << maxSize);
  //! if we send data...
...
MpTcpSubflow::AddMpTcpOptions (TcpHeader& header)
{
    ...
    // First if block added by kashif
      
    //if( (header.GetFlags () & TcpHeader::SYN && GetMeta()->FullyEstablished()))
     //{ std::cout<<" calling AddOptionMpTcpjoin(header) to append join options "<<std::endl;
      //AddOptionMpTcpjoin(header);
     //}
    //else 
...
MpTcpSubflow::CompleteFork(Ptr<Packet> p, const TcpHeader& h, const Address& fromAddress, const Address& toAddress)
{
  ...
  if(IsMaster()) //if condition added by kashif
    {
...
//This function is added by kashif to call subflow endpoint to handle receive packet
int
...
MpTcpSubflow::AddOptionMpTcp3WHS(TcpHeader& hdr) const
{
  ...
  //if( !IsMaster() ) {  std::cout<<" ismaster false"<<std::endl; }
...
//    }
  ...
  {   //std::cout<<" from mptcp-subflow adding join "<<std::endl;
...
//          MpTcpSocketBase::GenerateTokenForKey( MPTCP_SHA1, GetMeta()->GetRemoteKey(), token, idsn );
  ...
  //        id = GetIdManager()->GetLocalAddrId( InetSocketAddress(m_endPoint->GetLocalAddress(),m_endPoint->GetLocalPort()) );
         ...
         // std::cout<<" attached Hmac "<<std::endl;
          
          join->SetNonce(4242); //! truly random :)
          //std::cout<<" attached Nonce "<<std::endl;
...
// added by kashif to append join option
void
MpTcpSubflow::AddOptionMpTcpjoin(TcpHeader& hdr) const
{

 // std::cout<<" inside AddOptionMpTcpjoin "<<std::endl;
  Ptr<TcpOptionMpTcpJoin> join =  CreateObject<TcpOptionMpTcpJoin>();
      ...
      case (TcpHeader::SYN | TcpHeader::ACK):
        {
          ...
          //! TODO request from idmanager an id
          static uint8_t id = 0;
          const uint64_t HMACB = 424242;
          // TODO
          NS_LOG_WARN("IDs are incremental, there is no real logic behind it yet");
  //        id = GetIdManager()->GetLocalAddrId( InetSocketAddress(m_endPoint->GetLocalAddress(),m_endPoint->GetLocalPort()) );
          join->SetAddressId( id++ );
          join->SetTruncatedHmac(HMACB); // who cares
          join->SetNonce(4242); //! truly random :)
        }
        break;
...
// This function to pass peer token to tcpsocketbase, added by kashif
uint32_t
