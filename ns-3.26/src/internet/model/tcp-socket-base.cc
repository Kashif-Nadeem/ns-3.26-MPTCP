    ...
    case CLOSED:
      ...
      if ((tcpHeader.GetFlags () & ~(TcpHeader::PSH | TcpHeader::URG)) != TcpHeader::RST)
        { // Since m_endPoint is not configured yet, we cannot use SendRST here
          ...
          if (m_mptcpEnabled)  // if block added by kashif
          {
           SendPacket(h, p);
          }
          else 
...
TcpSocketBase::ProcessEstablished (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (tcpflags == TcpHeader::ACK)
    {
      ProcessTcpOptions(tcpHeader);   // co
...
TcpSocketBase::LimitedTransmit ()
{
  ...
  //added the following condition for mptcp by kashif
      uint32_t sz = 0;
      if (m_mptcpEnabled)
           {  
               TcpHeader header;
               GenerateEmptyPacketHeader(header, 0);
               sz = SendDataPacket (header, m_tcb->m_nextTxSequence, m_tcb->m_segmentSize);
           }
      else {
               sz = SendDataPacket (m_tcb->m_nextTxSequence, m_tcb->m_segmentSize, true);
          }
      ...
      if (m_tcb->m_congState == TcpSocketState::CA_OPEN)
      ...
      else if (m_tcb->m_congState == TcpSocketState::CA_RECOVERY)
        {
          if (ackNumber < m_recover)
            { 
              m_firstTxUnack = ackNumber;  //copied from ns323 by kashif
              /* Partial ACK.
      ...
      if (callCongestionControl)
        {
          ...
          std::cout<<" "<< m_endPoint->GetLocalPort () << ":"<<m_endPoint->GetPeerPort ();
          std::cout<< " " <<
                        " cWnd: " << m_tcb->m_cWnd <<
                        " ssTh: " << m_tcb->m_ssThresh<<std::endl;
                              ...
                              const Address& fromAddress, const Address& toAddress)
{
  ...
  if (!NotifyConnectionRequest (fromAddress))
   ...
   // If it is a SYN packet with an MP_JOIN option [ if condition by kashif]
    Ptr<const TcpOptionMpTcpJoin> join;
...
//        master->Bind();
      ...
      m_firstTxUnack = m_tcb->m_nextTxSequence;             // copied from ns323 by kashif
      SendEmptyPacket (TcpHeader::ACK);
  ...
  else if (tcpflags == (TcpHeader::SYN | TcpHeader::ACK) // This block added by kashif for MP_join handshake
           && GetTcpOption(tcpHeader, join))
    {
                               ...
                               const Address& fromAddress, const Address& toAddress)
{
  ...
  if (tcpflags == 0
    ...
    { // If it is bare data, accept it and move to ESTABLISHED state. This is
      ...
      m_tcb->m_highTxMark = ++m_tcb->m_nextTxSequence;
      m_txBuffer->SetHeadSequence (m_tcb->m_nextTxSequence);
      m_firstTxUnack = m_tcb->m_nextTxSequence;  // copied from ns3.23 added by kashif 
      if (m_endPoint)
        {
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
 ...
 if (m_mptcpEnabled)    // if block added by kashif date 9/11/17
 { 
       TcpHeader header;
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
 ...
 if (m_mptcpEnabled)         //added by kashif for mptcp
 { 
       TcpHeader h;  
       GenerateEmptyPacketHeader(h, withAck ? TcpHeader::ACK : 0);
       return SendDataPacket (h, seq, maxSize);
 }
...
// This function preserves functionality of 
//ns3.23 SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck) and
//ns3.23 SendDataPacket (TcpHeader& header, SequenceNumber32 seq, uint32_t maxSize)
uint32_t
TcpSocketBase::SendDataPacket (TcpHeader& header, SequenceNumber32 seq, uint32_t maxSize)
{
  NS_LOG_FUNCTION (this << header << seq << maxSize);

  bool isRetransmission = false;
  if (seq != m_tcb->m_highTxMark) // in ns3.22 this line is if ( seq == FirstUnackedSeq() )
    {
      isRetransmission = true;
    }
...
// added from ns3.23 and modified by kashif
void
...
//NS_ASSERT(header.GetWindowSize() == AdvertisedWindowSize ()); // commented by kashif
  ...
  while (m_txBuffer->SizeFromSequence (m_tcb->m_nextTxSequence))
    {
      ...
      if (w < m_tcb->m_segmentSize && m_txBuffer->SizeFromSequence (m_tcb->m_nextTxSequence) > w)
        { 
          NS_LOG_LOGIC ("Preventing Silly Window Syndrome. Wait to send." <<" Segment size"<< m_tcb->m_segmentSize<<" size from sequene"<< m_txBuffer->SizeFromSequence (m_tcb->m_nextTxSequence) <<"available window "<< w);
          break; // No more
        }
...
TcpSocketBase::NewAck (SequenceNumber32 const& ack, bool resetRTO)
{
  ...
  m_firstTxUnack = ack;          // coppied from ns3.23
  if (GetTxAvailable () > 0)
    {
      NotifySend (GetTxAvailable ());
    }
  ...
  if (m_txBuffer->Size () == 0 && m_state != FIN_WAIT_1 && m_state != CLOSING)
   ...
   // Try to send more data....
 // if (!m_sendPendingDataEvent.IsRunning ())  //added from ns323
   // {
...
TcpSocketBase::DoRetransmit ()
{
  ...
  // in ns3.23 m_txBuffer->HeadSequence () is replaced by FirstUnackedSeq()
...
//  NS_ASSERT_MSG( m_mptcpLocalKey != 0, "Key already generated");
  ...
  std::cout<<" localtoken & peertoken "<<m_mptcpLocalToken<<" "<<m_mptcpPeerToken<<std::endl;
...
TcpSocketBase::AddMpTcpOptions (TcpHeader& header)
{
    ...
    {
        std::cout<<" adding MP_capable to the header "<<std::endl;
    ...
    // first if block added by kashif to append join option to SYN packet to create subflow
     if(m_state == ESTABLISHED && (header.GetFlags () == TcpHeader::SYN)) 
    { 
         
       std::cout<<" adding mp_join to the header from tcpsocketbase "<<std::endl;
...
//          MpTcpSocketBase::GenerateTokenForKey( MPTCP_SHA1, GetMeta()->GetRemoteKey(), token, idsn );
          m_mptcpPeerToken = 887; //meta->GetMeta()->GetPeerToken();
      
       join->SetPeerToken(m_mptcpPeerToken); //join->SetPeerToken(GetMeta()->GetPeerToken());
       join->SetNonce(0);
...
{
  NS_LOG_FUNCTION (this << header);
  if(IsTcpOptionAllowed(TcpOption::MPTCP))    // copied from ns323 by kashif
  {
    NS_LOG_DEBUG("MPTCP enabled/check");
    AddMpTcpOptions(header);
  }

  // The window scaling option is set only on SYN packets //copied from ns-3.23 by kashif
