...
MpTcpSocketBase::GetTypeId(void)
{
  static TypeId tid = TypeId("ns3::MpTcpSocketBase")
     ...
     //     UintegerValue(8),
     //     MakeUintegerAccessor(&MpTcpSocketBase::SetMaxSubFlowNumber),
...
MpTcpSocketBase::Send(Ptr<Packet> p, uint32_t flags)
{
 ...
 // this line added by kashif for BulkSendApplication which calls directly Send
...
//        << " (context=" << context << ")"
  ...
  //std::cout<<"Subflow updated window from " << oldCwnd << " to " << newCwnd<<std::endl;
...
//  check we can accept the creation of a new subflow (did we receive a DSS already ?)
  ...
  //std::cout<<" return new subflow endpoint "<<subflow->m_endPoint<<std::endl;

  return subflow->m_endPoint;
}


/* To Initiate multiple new subflows with different source and destination port pairs
  added by kashif */
void
MpTcpSocketBase::InitiateMultipleSubflows()
{
  NS_LOG_FUNCTION_NOARGS();
  //std::cout<<" InitiateMultipleSubflows() called "<<std::endl;
  ...
  for (int i = 0; i < maxSubflows; i++)
    {
      uint16_t RandomSourcePort = 0;
      uint16_t RandomDestinationPort = 0;
      RandomSourcePort = rand() % 65000;
      RandomDestinationPort = rand() % 65000;
      if (RandomSourcePort == m_endPoint->GetLocalPort())
      {
       ...
       if (RandomDestinationPort == m_endPoint->GetLocalPort())
      {
       NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " [" << m_node->GetId() << "] This is a rear condition where a generated random port is the same as initial subflow's local port!");
       RandomDestinationPort++;
       }

      do
        {
          //randomGap = uniRandom->GetInteger(0, 50);
          randomGap = rand() % 50; // m_rGap = 50
        }
      ...
      InitiateSingleSubflows(RandomSourcePort, RandomDestinationPort); 
    }
}

/* To Initiate new subflow with different source and destination port pairs
  added by kashif */
bool
MpTcpSocketBase::InitiateSingleSubflows(uint16_t RandomSourcePort, uint16_t RandomDestinationPort)
{
  NS_LOG_FUNCTION_NOARGS();
  Ptr<MpTcpSubflow> subflow;
  NS_LOG_DEBUG("from Initiatesinglesubflow " << this << " "<< GetInstanceTypeId());
  //m_subflowTypeId = MpTcpSubflow::GetTypeId();
  ...
  std::cout<<" sAddr "<< sAddr<<" sPort "<<sPort<< " dAddr  "<<dAddr<<"  dPort "<<dPort<<std::endl;
  subflow->m_tcb->m_cWnd = m_tcb->m_segmentSize;
  //NS_LOG_INFO ("subflow State Changed from "<< TcpStateName[m_state] << " -> SYN_SENT");
  ...
  subflow->m_endPoint = m_tcp->Allocate(sAddr, sPort, dAddr, dPort);
  ...
  NS_LOG_UNCOND(this << " => "<< Simulator::Now().GetSeconds() <<" [" << m_node->GetId() <<"] (" <<"sFlow->routeId "<< ") InitiateSingleSubflow -> 4-Tuple: " << sAddr<< ":"<< sPort << " , "<< dAddr << ":" << dPort);
...
MpTcpSocketBase::SendPendingData(bool withAck)
{
  ...
  if(FullyEstablished() )
  {
     if(!addrAdvertised)
     { 
      //host can initiate subflows as soon as DSS option with a data ack is received
    
      //std::cout<<" calling initiatemultiplesubflows from sendpending data "<<std::endl;
      InitiateMultipleSubflows();
      addrAdvertised = true;
     }
   }  
   // std::cout<<" active subflows "<<GetNActiveSubflows()<<std::endl;    
...
//  for(int i = 0; i < (int)GetNActiveSubflows() ; i++ )
  //  {
    //    Ptr<MpTcpSubflow> sf = GetSubflow(i);
      //  std::cout<<" here are established subflows"<<sf<<std::endl;
   // }
...
/* This function is added to access m_nextTxSequence in 
         MpTcpSchedulerRoundRobin class by kashif */
...
//               m_tcb->m_cWnd.Get()
 ...
 //std::cout<<" "<< m_endPoint->GetLocalPort () << " : "<<m_endPoint->GetPeerPort ()<<std::endl;
 //std::cout<<"Total congestion window before computation " << Window()<< std::endl;
...
//          totalCwnd += sf->m_cWnd.Get();          // Should be this all the time ?!
  ...
  //std::cout<<" cwnd after computation "<<m_tcb->m_cWnd.Get()<<std::endl;
  return totalCwnd;
}
