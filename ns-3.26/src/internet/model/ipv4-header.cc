...
Ipv4Header::SetDscp (DscpType dscp)
{
  ...
  m_tos &= 0x03; // Clear out the DSCP part, retain 2 bits of ECN // replaced 0x3 by 0x03 [Kashif modified]
  m_tos |= (dscp << 2);
}
...
Ipv4Header::GetEcn (void) const
{
  ...
  return EcnType (m_tos & 0x03);  // modified from 0x3 to 0x03 [kashif]
}
