...
TcpL4Protocol::DoDispose (void)
{
  ...
  if (m_endPoints != 0)
    {
      //delete m_endPoints;  commented by kashif because of memory errors
      m_endPoints = 0;
    }
...
//addr->~MpTcpSocketBase();
  NS_LOG_UNCOND(
                ...
                << "sizeof(aligned mtcp)=" << sizeof(MpTcpSocketBase)
  ...
  **/
  ...
  {
       char *addr = new char[ sizeof(MpTcpSocketBase)];
  ...
  {
    //std::cout<<" we gonna create socket for new subflow "<<std::endl;
                               ...
                               const Address &source, const Address &destination)
{
 ...
 // std::cout<<"Tcp received a packet "<<m<<" at "<<Simulator::Now ().GetSeconds()
...
{  
  ...
  NS_LOG_FUNCTION (this << packet << incomingIpHeader << incomingInterface); //m++;
  ...
  **/
  ...
  // if (endPoints.empty()) {std::cout<<"  end point empty "<<std::endl;}
   // else { std::cout<<"  end point not empty "<< incomingTcpHeader.GetSourcePort () <<" > "<< incomingTcpHeader.GetDestinationPort ()<<std::endl; }

  if (endPoints.empty())
  {
      NS_LOG_LOGIC ("No Ipv4 endpoints matched on TcpL4Protocol, "
                    "checking if packet is a MP_JOIN request:" << incomingIpHeader);
      //std::cout<<" endpoints are"<<m_endPoints<<std::endl;
...
//        (*endPoints.begin())->ForwardUp(packet, ipHeader, tcpHeader.GetSourcePort(), incomingInterface);
  ...
  //for new subflows need to find out endpoint/subflow in the container

  (*endPoints.begin ())->ForwardUp (packet, incomingIpHeader,
                                    incomingTcpHeader.GetSourcePort (),
                                    incomingInterface);
  // std::cout<<" forwarded upto endpoint/socket"<<std::endl;
                        ...
                        Ptr<Ipv6Interface> interface)
{
  ...
  //std::cout<<"Tcp received packet "<<m<<" at "<<Simulator::Now ().GetSeconds()
   //                              << " data size " << packet->GetSize ()<<std::endl;
  NS_LOG_FUNCTION (this << packet << incomingIpHeader.GetSourceAddress () <<
                   incomingIpHeader.GetDestinationAddress ()); m++;
                             ...
                             Ptr<NetDevice> oif) const
{
  ...
  //std::cout<<"Tcp Sending packet "<<k<<" at "<<Simulator::Now ().GetSeconds()
             //                    << " data size " << packet->GetSize ()<<std::endl; k++;
  NS_LOG_LOGIC ("TcpL4Protocol " << this
...
TcpL4Protocol::AddSocket (Ptr<TcpSocketBase> socket)
{
  ...
  DumpSockets(); // added new by kashif from ns3.23
