 ...
 */
#ifndef MY_FIRST_HELPER_H
#define MY_FIRST_HELPER_H
 ...
 */
class MyFirstServerHelper
{
public:
  /**
   * Create MyFirstServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  MyFirstServerHelper (uint16_t port);
  ...
  /**
   * Create a MyFirstServerApplication on the specified Node.
   *
  ...
  /**
   * Create a MyFirstServerApplication on specified node
   *
   ...
   *
   * Create one my first server application on each of the Nodes in the
   * NodeContainer.
   *
...
private:
  /**
   * Install an ns3::MyFirstServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an MyFirstServer will be installed.
 ...
 */
class MyFirstClientHelper
{
public:
  /**
   * Create MyFirstClientHelper which will make life easier for people trying
   ...
   */
  MyFirstClientHelper (Address ip, uint16_t port);
  /**
   * Create MyFirstClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   * include a port value (e.g., InetSocketAddress and Inet6SocketAddress).
   *
   * \param addr The address of the remote my first server
   */
  MyFirstClientHelper (Address addr);
  ...
  /**
   * Given a pointer to a MyFirstClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be MyFirstClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the MyFirstClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the MyFirstClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::MyFirstClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an MyFirstClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
