...
public:
   ...
   /**
    * coppied from kheirkha code by kashif 
    * to initiate multiple subflows
    * Mathieu Coudron code doesn't create subflows other than meta flow
    */
   typedef enum
  {
  //Default,
  ...
  NdiffPorts
   ...
   bool InitiateSingleSubflows(uint16_t RandomSourcePort, uint16_t RandomDestinationPort); // Initiate new subflows when nDiffPorts is active
...
private:
  ...
  uint8_t  maxSubflows;    // max number of subflows to generate
  uint32_t m_rGap;
  bool addrAdvertised;
